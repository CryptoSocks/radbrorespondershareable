...
def sleeper_v2(limit, request_list):
    ...
    if len(request_list) >= limit:
        ...
        print("Rate limit hit eepy now " + current_time.strftime("%d/%m/%Y %H:%M:%S"))
...
def tweet(text, tweet_id, user_id, media_key, tweeted, liked):
    ...
    if user_id != RESPONDER_ID:
        ...
        print("Tweets in the last 15 min: " + str(len(last_tweet_request)))
        sleeper_v2(200, last_tweet_request)
        if media_key:
        ...
        if not liked:
            print("Likes in the last 15 min: " + str(len(last_like_request)))
            sleeper_v2(5, last_like_request)
